...
def seed_supabase():
    ...
    try:
        ...
        for i, table in enumerate(tables):
            ...
            # Skip SQLite-only columns not in PostgreSQL schema
            SKIP_COLS = {"telefono","dni","puesto","area_trabajo","sqlite_sequence"}
            cols_filtered = [c for c in cols if c not in SKIP_COLS]
            cols_str = ", ".join(cols_filtered)
            placeholders = ", ".join(["%s"] * len(cols_filtered))
            col_indices = [cols.index(c) for c in cols_filtered]
            ...
            with conn.cursor() as cur:
                for j in range(0, len(rows), 50):
                    batch = rows[j:j+50]
                    batch_filtered = [[row[i] for i in col_indices] for row in batch]
                    cur.executemany(sql, batch_filtered)
            conn.commit()
            conn.close()
