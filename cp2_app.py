...
if _USE_SUPABASE:
    ...
    def _pg_dsn():
        ...
        return (f"postgresql://postgres.{project_id}:{_SUPABASE_DB_PASS}"
    ...
    def _get_pg_pool():
        ...
        return pool.ThreadedConnectionPool(1, 10, _pg_dsn())
    ...
    def _fix_sql(sql):
        ...
        def _sf(m):
            ...
            col = m.group(2).strip()
            pg = fmt
            pg = pg.replace('%Y-%m-%d %H:%M','YYYY-MM-DD HH24:MI')
            pg = pg.replace('%Y-%m-%d','YYYY-MM-DD')
            pg = pg.replace('%Y-%m','YYYY-MM')
            pg = pg.replace('%Y%m','YYYYMM')
            pg = pg.replace('%d/%m/%Y','DD/MM/YYYY')
            pg = pg.replace('%d/%m','DD/MM')
            pg = pg.replace('%Y','YYYY')
            pg = pg.replace('%m','MM')
            pg = pg.replace('%d','DD')
            stripped = col.strip().strip(chr(39)).strip(chr(34))
            if stripped == 'now':
                return 'TO_CHAR(NOW(), ' + chr(39) + pg + chr(39) + ')'
            return 'TO_CHAR(' + col + '::timestamp, ' + chr(39) + pg + chr(39) + ')'
        pat = r'strftime\s*\(\s*[' + chr(39) + chr(34) + r']([^' + chr(39) + chr(34) + r']+)[' + chr(39) + chr(34) + r']\s*,\s*([^)]+)\)'
        sql = _re2.sub(pat, _sf, sql)
        sql = sql.replace("date('now')", 'CURRENT_DATE::TEXT')
        sql = sql.replace('date("now")', 'CURRENT_DATE::TEXT')
        sql = sql.replace("datetime('now')", 'NOW()::TEXT')
        sql = sql.replace('datetime("now")', 'NOW()::TEXT')
        # Fix LIKE with SQLite wildcards - already standard
        # Fix iif() -> CASE WHEN
        sql = _re2.sub(r'iif\(([^,]+),([^,]+),([^)]+)\)', r'CASE WHEN  THEN  ELSE  END', sql)
        sql = sql.replace('?', '%s')
        return sql
    ...
    def qdf(sql, params=()):
        ...
        conn = get_conn()
        try:
            result = pd.read_sql_query(pg_sql, conn, params=list(params))
            return result
        finally:
            release_conn(conn)
...
def seed_supabase():
    """Carga datos iniciales en Supabase tabla por tabla con reintentos"""
    ...
    try:
        ...
        # Check if seed already completed using configuracion table as flag
        tables = list(all_data.keys())
        try:
            flag = qdf("SELECT valor FROM configuracion WHERE clave='seed_completed'")
            if len(flag) > 0 and flag.iloc[0]["valor"] == "1":
                return  # Already seeded
        except:
            pass

        progress = st.progress(0, text="Cargando base de datos...")
        ...
        for i, table in enumerate(tables):
            ...
            try:
                ...
                if check.iloc[0]["n"] > 0:
                    pct = (i+1)/len(tables)
                    progress.progress(pct, text=f"✓ {table} ya tiene datos")
                    continue
            except:
                pass

            cols_filtered = [c for c in cols]
            cols_str = ", ".join(cols_filtered)
            placeholders = ", ".join(["%s"] * len(cols_filtered))
            col_indices = list(range(len(cols_filtered)))
            sql = f"INSERT INTO {table} ({cols_str}) VALUES ({placeholders}) ON CONFLICT DO NOTHING"

            # Insert in small batches with individual connections
            batch_size = 10  # Very small to avoid timeout
            total_batches = len(range(0, len(rows), batch_size))

            for b, j in enumerate(range(0, len(rows), batch_size)):
                batch = rows[j:j+batch_size]
                batch_data = [[row[k] for k in col_indices] for row in batch]
                ...
                for attempt in range(3):
                    try:
                        ...
                        with conn2.cursor() as cur2:
                            cur2.executemany(sql, batch_data)
                        conn2.commit()
                        conn2.close()
                        break
                ...
                pct = (i + (b+1)/total_batches) / len(tables)
                progress.progress(min(pct, 0.99),
                    text=f"Cargando {table}: {min(j+batch_size, len(rows))}/{len(rows)}")
        ...
        try:
            run("INSERT INTO configuracion (clave,valor,descripcion) VALUES (?,?,?) ON CONFLICT (clave) DO UPDATE SET valor=?",
                ("seed_completed","1","Seed inicial completado","1"))
        except:
            pass

        progress.progress(1.0, text="✅ Base de datos lista")
        time.sleep(2)
        progress.empty()
        st.rerun()

    except Exception as e:
        st.warning(f"Error cargando datos: {e} — Recargá la página para continuar")
